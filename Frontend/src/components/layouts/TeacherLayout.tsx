...
const sidebarItems = [
  ...
  { name: "Study Materials", icon: Upload, href: "/instructor/materials" },
  { name: "Discussions", icon: MessageSquare, href: "/instructor/discussions" },
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <motion.div
                    className="w-9 h-9 rounded-full bg-accent/10 flex items-center justify-center cursor-pointer overflow-hidden border border-accent/20"
                    whileHover={{ scale: 1.1, rotate: 5 }}
                    whileTap={{ scale: 0.95 }}
                  >
                    <Avatar className="h-9 w-9">
                      <AvatarImage src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${user?.email}`} alt={user?.name} />
                      <AvatarFallback className="bg-accent/10 text-accent font-medium">{initials}</AvatarFallback>
                    </Avatar>
                  </motion.div>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" className="w-56">
                  <DropdownMenuLabel>
                    <div className="flex flex-col space-y-1">
                      <p className="text-sm font-medium leading-none">{user?.name || 'Instructor'}</p>
                      <p className="text-xs leading-none text-muted-foreground">{user?.email}</p>
                    </div>
                  </DropdownMenuLabel>
                  <DropdownMenuSeparator />
                  <DropdownMenuItem asChild>
                    <Link to="/instructor/profile" className="cursor-pointer">
